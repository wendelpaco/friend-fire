...
export function ServerBrowser({ onClose }: ServerBrowserProps) {
 ...
 return (
  ...
  >
   <div className="flex max-h-[90vh] w-full max-w-3xl flex-col border-2 border-[rgba(255,179,0,0.20)] bg-[#12151A] p-6 shadow-[0_16px_40px_rgba(0,0,0,0.7)]">
    ...
    <div className="mt-4 flex flex-wrap items-center gap-2">
     <div
      className="flex items-center gap-0.5 border-2 border-[rgba(255,179,0,0.16)] bg-black/40 p-0.5"
      role="group"
     ...
     <label className="flex items-center gap-2 text-xs text-[#B9B29F]">
      <span className="sr-only">Mapa</span>
      <select
       value={mapFilter}
       onChange={(e) => setMapFilter(e.target.value)}
       className=" border-2 border-[rgba(255,179,0,0.16)] bg-black/50 px-2.5 py-1.5 text-xs font-semibold text-[#F4EFE3] outline-none focus:border-amber-400/50"
       aria-label="Filtrar por mapa"
      >
       <option value="" className="bg-[#12151A]">
        Todos os mapas
       </option>
       {FILTER_MAPS.map((m) => (
        <option key={m.id} value={m.id} className="bg-[#12151A]">
         {m.label}
        </option>
       ))}
      </select>
     </label>
     <label className="flex cursor-pointer items-center gap-2 text-xs text-[#6F6A5B]">
      <input
       type="checkbox"
     ...
     <button
      ...
      className=" border-2 border-[rgba(255,179,0,0.16)] bg-white/[0.04] px-3 py-1.5 text-xs font-semibold tracking-wide text-[#F4EFE3] transition hover:bg-white/[0.08] disabled:opacity-50"
     >
     ...
     <span className="ml-auto text-[10px] tabular-nums text-[#6F6A5B]">
    ...
    <div className="mt-4 min-h-0 flex-1 overflow-auto border-2 border-[rgba(255,179,0,0.16)]">
     {loading && rooms.length === 0 && !error ? (
      <p className="px-4 py-10 text-center text-sm text-[#6F6A5B]">
       Carregando salas…
      </p>
     ) : rooms.length === 0 ? (
      <div className="px-4 py-10 text-center">
       <p className="text-sm font-semibold text-[#B9B29F]">
        Nenhuma sala aberta
       </p>
       <p className="mt-1 text-xs text-[#6F6A5B]">
      ...
      <table className="w-full min-w-[560px] border-collapse text-left text-sm">
       <thead className="sticky top-0 bg-[#141820] text-[10px] font-semibold uppercase tracking-[0.18em] text-[#6F6A5B]">
        <tr>
       ...
       <tbody>
        {rooms.map((room) => {
         ...
         return (
          <tr
           key={key}
           className="border-t border-white/5 text-[#F4EFE3] transition hover:bg-white/[0.03]"
          >
           <td className="max-w-[10rem] truncate px-3 py-2.5 font-medium text-[#F4EFE3]">
            {roomDisplayName(room)}
           </td>
           <td className="px-3 py-2.5">
            <span
             className="inline-flex max-w-[10rem] items-center gap-1.5 truncate rounded-md border-2 border-[rgba(255,179,0,0.16)] bg-white/[0.04] px-1.5 py-0.5 text-xs font-semibold text-[#F4EFE3]"
             title={chip.label}
            >
             <span
            ...
            <span
             className={`inline-flex rounded-md border px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide ${
              vis === "public"
               ...
               : "border-white/15 bg-white/[0.04] text-[#B9B29F]"
             }`}
            >
           ...
           <td className="px-3 py-2.5 text-right">
            <button
             ...
             className={` px-3 py-1.5 text-xs font-bold tracking-wide transition disabled:cursor-not-allowed disabled:opacity-40 ${
              full
               ? "border-2 border-[rgba(255,179,0,0.16)] bg-white/[0.04] text-[#6F6A5B]"
    ...
    <button
     type="button"
     onClick={onClose}
     className="mt-4 border-2 border-[rgba(255,179,0,0.16)] bg-white/[0.04] py-3 text-sm font-semibold text-[#B9B29F] transition hover:bg-white/[0.08]"
    >
     Fechar
    </button>
   </div>
